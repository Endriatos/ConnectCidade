...
SOLICITACAO_BASE = {
    "id_categoria": 1,
    "descricao": "Poste apagado na esquina",
    "endereco_referencia": "Rua dos Apoios, 10",
    "latitude": -23.561,
    "longitude": -46.655,
...
def _gerar_cpf(seed: int) -> str:
    ...
    Os 9 primeiros dígitos são derivados do seed (preenchido com zeros à esquerda).
    Os dois dígitos verificadores são calculados pelo algoritmo da Receita Federal:
    - 1º dígito: soma ponderada (pesos 10..2) mod 11; resto < 2 → 0, senão 11 - resto.
    - 2º dígito: soma ponderada (pesos 11..2) mod 11; mesmo critério.
    Garante CPFs únicos e matematicamente válidos para cada seed distinto.
    """
    ...
    # Calcula o segundo dígito verificador (inclui o primeiro)
...
def _criar_solicitacao(client, token: str) -> int:
    """Cria uma solicitação com dados fixos e retorna o id_solicitacao."""
    resp = client.post(
        "/solicitacoes",
